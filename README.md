...
GCN-based recommenders capture high-order user–item relations, but on long-tail data their message passing amplifies popularity bias: high-degree items accumulate larger gradients and develop inflated embedding norms, so they dominate recommendations regardless of directional relevance. Cosine-similarity remedies remove this magnitude bias but also discard user embedding norms that encode activity levels, and contrastive remedies add costly augmentation without addressing norm disparity directly. To address these limitations, we propose **DNA** (Debiasing with Norm Adjustment). DNA normalizes only item embeddings to a fixed scale, preserving user magnitudes, and adds a learnable item-specific bias trained outside the GCN so it restores popularity signals without re-learning the amplified bias. DNA plugs into any GCN backbone without architectural changes, consistently improves LightGCN, IMPGCN, LayerGCN, SimGCL and XSimGCL on Yelp, Gowalla and Amazon-CD, and raises the recommendation ratio of low-degree items from 0.38% to 2.06% on Amazon-CD.
